...
scroll_CODE.bin: scroll.c draw_dirty_edge.c copy_viewport_32x16.asm dixel_scroll.asm tiles_extern.asm hud_data.asm hud.scr
	PATH=$(Z88DK)/bin:$$PATH Z88DK=$(Z88DK) ZCCCFG=$(ZCCCFG) $(ZCC) $(CFLAGS) $(USER_CFLAGS) -o scroll scroll.c draw_dirty_edge.c copy_viewport_32x16.asm dixel_scroll.asm tiles_extern.asm hud_data.asm -lm
